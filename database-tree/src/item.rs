...
pub enum DatabaseTreeItemKind {
  ...
  Schema { database: Database, schema: Schema, collapsed: bool },
}
...
/// `DatabaseTreeItem` can be of two kinds: see `DatabaseTreeItem` but shares an
/// info
#[derive(Debug, Clone)]
...
impl DatabaseTreeItem {
  pub fn new_table(database: &Database, table: &Table) -> Self {
    Self {
      ...
      kind: DatabaseTreeItemKind::Table { database: database.clone(), table: table.clone() },
    }
  }
  ...
  pub fn new_database(database: &Database, _collapsed: bool) -> Self {
    Self {
      ...
      kind: DatabaseTreeItemKind::Database { name: database.name.to_string(), collapsed: true },
    }
  }

  pub fn set_collapsed(&mut self, collapsed: bool) {
    if let DatabaseTreeItemKind::Database { name, .. } = self.kind() {
      self.kind = DatabaseTreeItemKind::Database { name: name.to_string(), collapsed }
    }
  }
  ...
  pub fn collapse_database(&mut self) {
    if let DatabaseTreeItemKind::Database { name, .. } = &self.kind {
      self.kind = DatabaseTreeItemKind::Database { name: name.to_string(), collapsed: true }
    }
  }

  pub fn expand_database(&mut self) {
    if let DatabaseTreeItemKind::Database { name, .. } = &self.kind {
      self.kind = DatabaseTreeItemKind::Database { name: name.to_string(), collapsed: false };
    }
  }

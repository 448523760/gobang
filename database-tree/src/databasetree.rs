...
impl DatabaseTree {
  ...
  pub fn selected_item(&self) -> Option<&crate::DatabaseTreeItem> {
    self.selection.and_then(|index| self.items.tree_items.get(index))
  }
  ...
  pub fn selected_table(&self) -> Option<(Database, Table)> {
    self.selection.and_then(|index| {
      ...
      match item.kind() {
        ...
        DatabaseTreeItemKind::Table { table, database } => Some((database.clone(), table.clone())),
  ...
  fn calc_visual_selection(&self) -> Option<VisualSelection> {
    self.selection.map(|selection_absolute| {
      ...
      VisualSelection { index: visual_index, count }
    })
  }
  ...
  fn selection_down(&self, current_index: usize, lines: usize) -> Option<usize> {
    let mut index = current_index;
    let last_visible_item_index = self.items.tree_items.iter().rposition(|x| x.info().is_visible())?;
  ...
  fn selection_updown(&self, current_index: usize, up: bool) -> Option<usize> {
    ...
    loop {
      index = {
        let new_index = if up { index.saturating_sub(1) } else { index.saturating_add(1) };
  ...
  fn is_visible_index(&self, index: usize) -> bool {
    self.items.tree_items.get(index).map(|item| item.info().is_visible()).unwrap_or_default()
  }
}
...
mod test {
  ...
  impl Table {
    fn new(name: String) -> Self {
      Table { name, create_time: None, update_time: None, engine: None, schema: None }
    }

    fn new_with_schema(name: String, schema: String) -> Self {
      Table { name, create_time: None, update_time: None, engine: None, schema: Some(schema) }
    }
  }

  #[test]
  fn test_selection() {
    let items = vec![Database::new("a".to_string(), vec![Table::new("b".to_string()).into()])];
    ...
    let items = vec![Database::new("a".to_string(), vec![Schema {
      name: "b".to_string(),
  ...
  fn test_selection_skips_collapsed() {
    ...
    let items = vec![
      Database::new("a".to_string(), vec![Schema {
        name: "b".to_string(),
        tables: vec![Table::new("c".to_string()).into()],
      }
      .into()]),
      Database::new("d".to_string(), vec![Schema {
        name: "e".to_string(),
  ...
  fn test_selection_left_collapse() {
    let items = vec![Database::new("a".to_string(), vec![
  ...
  fn test_selection_top() {
    ...
    let items = vec![Database::new("a".to_string(), vec![Schema {
      name: "b".to_string(),
      tables: vec![Table::new("c".to_string()).into(), Table::new("d".to_string()).into()],
    }
    .into()])];

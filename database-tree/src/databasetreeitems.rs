...
impl DatabaseTreeItems {
  ...
  pub fn new(list: &[Database], collapsed: &BTreeSet<&String>) -> Result<Self> {
    Ok(Self { tree_items: Self::create_items(list, collapsed)? })
  }
  ...
  fn create_items(list: &[Database], collapsed: &BTreeSet<&String>) -> Result<Vec<DatabaseTreeItem>> {
  ...
  fn push_databases<'a>(
    ...
    // increase child count in parent node (the above ancenstor ignores the leaf
    // component)
    *items_added.entry(database.name.clone()).or_insert(0) += 1;
      ...
      if let Some(ref collapsed_item) = inner_collapsed {
        match collapsed_item {
          DatabaseTreeItemKind::Database { name, .. } => {
            if let DatabaseTreeItemKind::Schema { database, .. } = self.tree_items[i].kind().clone() {
              if database.name == *name {
                continue;
              }
            }
            if let DatabaseTreeItemKind::Table { database, .. } = self.tree_items[i].kind().clone() {
              if database.name == *name {
                continue;
              }
            }
          ...
          DatabaseTreeItemKind::Schema { schema, .. } => {
            if let DatabaseTreeItemKind::Table { table, .. } = self.tree_items[i].kind().clone() {

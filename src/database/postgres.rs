...
impl TableRow for Column {
  ...
  fn columns(&self) -> Vec<String> {
    vec![
      self.name.as_ref().map_or(String::new(), |name| name.to_string()),
      self.r#type.as_ref().map_or(String::new(), |r#type| r#type.to_string()),
      self.null.as_ref().map_or(String::new(), |null| null.to_string()),
      self.default.as_ref().map_or(String::new(), |default| default.to_string()),
      self.comment.as_ref().map_or(String::new(), |comment| comment.to_string()),
    ]
  }
}
...
impl TableRow for ForeignKey {
  fn fields(&self) -> Vec<String> {
    vec!["name".to_string(), "column_name".to_string(), "ref_table".to_string(), "ref_column".to_string()]
  }

  fn columns(&self) -> Vec<String> {
    vec![
      self.name.as_ref().map_or(String::new(), |name| name.to_string()),
      self.column_name.as_ref().map_or(String::new(), |r#type| r#type.to_string()),
      self.ref_table.as_ref().map_or(String::new(), |r#type| r#type.to_string()),
      self.ref_column.as_ref().map_or(String::new(), |r#type| r#type.to_string()),
    ]
  }
}
...
impl TableRow for Index {
  fn fields(&self) -> Vec<String> {
    vec!["name".to_string(), "column_name".to_string(), "type".to_string()]
  }

  fn columns(&self) -> Vec<String> {
    vec![
      self.name.as_ref().map_or(String::new(), |name| name.to_string()),
      self.column_name.as_ref().map_or(String::new(), |column_name| column_name.to_string()),
      self.r#type.as_ref().map_or(String::new(), |r#type| r#type.to_string()),
    ]
  }
}
...
impl Pool for PostgresPool {
  async fn execute(&self, query: &String) -> anyhow::Result<ExecuteResult> {
    ...
    if query.to_uppercase().starts_with("SELECT") {
      ...
      let mut records = vec![vec!["record1-1".to_string(), "record1-2".to_string()], vec![
        "record2-1".to_string(),
        "record2-2".to_string(),
      ]];
  ...
  async fn get_databases(&self) -> anyhow::Result<Vec<Database>> {
    ...
    let mut list = vec![];
    for db in databases {
      list.push(Database::new(db.clone(), self.get_tables(db.clone()).await?))
    }
    Ok(list)
  }
  ...
  async fn get_indexes(&self, _database: &Database, table: &Table) -> anyhow::Result<Vec<Box<dyn TableRow>>> {
...
impl PostgresPool {
  async fn get_json_records(
    ...
    Ok(vec![Value::String("some value".to_string()), Value::String("Another value".to_string())])
  }
}

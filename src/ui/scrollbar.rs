...
impl Widget for Scrollbar {
  fn render(self, area: Rect, buf: &mut Buffer) {
    ...
    let right = if self.inside { area.right().saturating_sub(1) } else { area.right() };
    if right <= area.left() {
      return;
    };

    let (bar_top, bar_height) = {
      let scrollbar_area = area.inner(&Margin { horizontal: 0, vertical: if self.border { 1 } else { 0 } });

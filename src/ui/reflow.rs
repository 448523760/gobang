...
impl<'a, 'b> LineComposer<'a> for WordWrapper<'a, 'b> {
  fn next_line(&mut self) -> Option<(&[StyledGrapheme<'a>], u16)> {
    ...
    let mut current_line_width =
      self.current_line.iter().map(|StyledGrapheme { symbol, .. }| -> u16 { symbol.width().cast() }).sum();
...
/// As src is a unicode str, start offset has to be calculated with each
/// character.
...
mod test {
  ...
  fn line_composer_long_sentence() {
    ...
    assert_eq!(word_wrapper_single_space, vec![
    ...
    assert_eq!(word_wrapper_multi_space, vec![
      "abcd efghij    klmno",
      "pabcd efgh     ijklm",
  ...
  /// Tests WordWrapper with words some of which exceed line length and some
  /// not.
  #[test]
  ...
  /// Tests an input starting with a letter, folowed by spaces - some of the
  ...
  fn line_composer_char_plus_lots_of_spaces() {
    ...
    let (word_wrapper, _) = run_composer(Composer::WordWrapper { trim: true }, text, width);
    let (line_truncator, _) = run_composer(Composer::LineTruncator, text, width);
    // What's happening below is: the first line gets consumed, trailing spaces
    // discarded, after 20 of which a word break occurs (probably shouldn't).
    // The second line break discards all whitespace. The result should probably
    // be vec!["a"] but it doesn't matter that much.
    assert_eq!(word_wrapper, vec![
      "a                   ",
      "                    ",
      "                    ",
  ...
  fn line_composer_word_wrapper_double_width_chars_mixed_with_spaces() {
    let width = 20;
    // Japanese seems not to use spaces but we should break on spaces anyway...
    // We're using it to test double-width chars.
    // You are more than welcome to add word boundary detection based of alterations
    // of hiragana and katakana...
    // This happens to also be a test case for mixed width because regular spaces
    // are single width.
  ...
  fn line_composer_word_wrapper_nbsp() {
    ...
    // Ensure that if the character was a regular space, it would be wrapped
    // differently.
  ...
  fn line_composer_word_wrapper_preserve_indentation_with_wrap() {
    ...
    let (word_wrapper, _) = run_composer(Composer::WordWrapper { trim: false }, text, width);
    assert_eq!(word_wrapper, vec!["AAA AAA AA", "AAA AA AAA", "AAA", " B", "  C", "   D"]);
  }
  ...
  fn line_composer_word_wrapper_preserve_indentation_lots_of_whitespace() {
    ...
    let text = "               4 Indent\n                 must wrap!";
    let (word_wrapper, _) = run_composer(Composer::WordWrapper { trim: false }, text, width);
    assert_eq!(word_wrapper, vec!["          ", "     4 Ind", "ent", "          ", "       mus", "t wrap!"]);
  }
}

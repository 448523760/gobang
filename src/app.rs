...
use crate::{
  ...
  components::{
    command, tab::Tab, CommandInfo, Component as _, ConnectionsComponent, DatabasesComponent, DrawableComponent as _,
    ErrorComponent, EventState, HelpComponent, PropertiesComponent, RecordTableComponent, SqlEditorComponent,
    StatefulDrawableComponent, TabComponent,
  },
  config::Config,
  database::{Pool, PostgresPool, RECORDS_LIMIT_PER_PAGE},
  event::Key,
};
...
impl App {
  ...
  pub fn draw<B: Backend>(&mut self, f: &mut Frame<'_, B>) -> anyhow::Result<()> {
    if let Focus::ConnectionList = self.focus {
      self.connections.draw(
        f,
        Layout::default().constraints([Constraint::Percentage(100)]).split(f.size())[0],
        false,
      )?;
    ...
    self.databases.draw(f, main_chunks[0], matches!(self.focus, Focus::DabataseList))?;
    ...
    match self.tab.selected_tab {
      Tab::Records => self.record_table.draw(f, right_chunks[1], matches!(self.focus, Focus::Table))?,
      Tab::Sql => {
        self.sql_editor.draw(f, right_chunks[1], matches!(self.focus, Focus::Table))?;
      },
      Tab::Properties => {
        self.properties.draw(f, right_chunks[1], matches!(self.focus, Focus::Table))?;
      },
    }
    self.error.draw(f, Rect::default(), false)?;
    self.help.draw(f, Rect::default(), false)?;
  ...
  fn commands(&self) -> Vec<CommandInfo> {
    let mut res = vec![
      ...
      CommandInfo::new(command::scroll_up_down_multiple_lines(&self.config.key_config)),
      CommandInfo::new(command::move_focus(&self.config.key_config)),
      CommandInfo::new(command::extend_or_shorten_widget_width(&self.config.key_config)),
    ];
  ...
  async fn update_databases(&mut self) -> anyhow::Result<()> {
    if let Some(conn) = self.connections.selected_connection() {
      ...
      self.pool = if conn.is_mysql() {
        ...
        Some(Box::new(PostgresPool::new(conn.database_url()?.as_str()).await?))
      } else {
        None
      };
      self.databases.update(conn, self.pool.as_ref().unwrap()).await?;
  ...
  async fn update_record_table(&mut self) -> anyhow::Result<()> {
    if let Some((database, table)) = self.databases.tree().selected_table() {
      let (headers, records) = self
        ...
        .await?;
      self.record_table.update(records, headers, database.clone(), table.clone());
    }
    Ok(())
  }
    ...
    match self.focus {
      ...
      Focus::DabataseList => {
        ...
        if key == self.config.key_config.enter && self.databases.tree_focused() {
          if let Some((database, table)) = self.databases.tree().selected_table() {
            self.record_table.reset();
            let (headers, records) = self.pool.as_ref().unwrap().get_records(&database, &table, 0, None).await?;
            self.record_table.update(records, headers, database.clone(), table.clone());
            self.properties.update(database.clone(), table.clone(), self.pool.as_ref().unwrap()).await?;
        ...
        match self.tab.selected_tab {
          ...
          Tab::Sql => {
            if self.sql_editor.event(key)?.is_consumed()
              || self.sql_editor.async_event(key, self.pool.as_ref().unwrap()).await?.is_consumed()
            {
              return Ok(EventState::Consumed);
            };
          },
  ...
  fn extend_or_shorten_widget_width(&mut self, key: Key) -> anyhow::Result<EventState> {
    if key == self.config.key_config.extend_or_shorten_widget_width_to_left {
      self.left_main_chunk_percentage = self.left_main_chunk_percentage.saturating_sub(5).max(15);
      return Ok(EventState::Consumed);
    } else if key == self.config.key_config.extend_or_shorten_widget_width_to_right {
...
mod test {
  ...
  fn test_extend_or_shorten_widget_width() {
    ...
    assert_eq!(app.extend_or_shorten_widget_width(Key::Char('>')).unwrap(), EventState::Consumed);

...
impl TableComponent {
  ...
  fn title(&self) -> String {
    self.table.as_ref().map_or(" - ".to_string(), |table| format!("{}.{}", table.0.name, table.1.name))
  }

  pub fn update(&mut self, rows: Vec<Vec<String>>, headers: Vec<String>, database: Database, table: DTable) {
  ...
  fn scroll_to_bottom(&mut self) {
    ...
    self.reset_selection();
    self.selected_row.select(Some(self.rows.len().saturating_sub(1)));
  }
    ...
    if let Some((x, y)) = self.selection_area_corner {
      ...
      return Some(
        self.rows[y.min(selected_row_index)..y.max(selected_row_index) + 1]
          .iter()
          .map(|row| row[x.min(self.selected_column)..x.max(self.selected_column) + 1].join(","))
          .collect::<Vec<String>>()
          .join("\n"),
      );
    }
    self.rows.get(self.selected_row.selected()?)?.get(self.selected_column).map(|cell| cell.to_string())
  }
  ...
  fn is_selected_cell(&self, row_index: usize, column_index: usize, selected_column_index: usize) -> bool {
    if let Some((x, y)) = self.selection_area_corner {
      let x_in_page = x.saturating_add(1).saturating_sub(self.column_page_start.get());
      return matches!(
  ...
  fn rows(&self, left: usize, right: usize) -> Vec<Vec<String>> {
    let rows = self.rows.iter().map(|row| row.to_vec()).collect::<Vec<Vec<String>>>();
    let mut new_rows: Vec<Vec<String>> = rows.iter().map(|row| row[left..right].to_vec()).collect();
  ...
  fn calculate_cell_widths(
    ...
    loop {
      let length = self
        .rows
        .iter()
        .map(|row| row.get(column_index).map_or(String::new(), |cell| cell.to_string()).width())
        .collect::<Vec<usize>>()
        .iter()
        .max()
        .map_or(3, |v| {
    ...
    let mut constraints =
      widths.iter().map(|(_, width)| Constraint::Length(*width as u16)).collect::<Vec<Constraint>>();
    if self.selected_column_index() != self.headers.len().saturating_sub(1)
      && column_index.saturating_sub(1) != self.headers.len().saturating_sub(1)
    {
    ...
    (
      self.selection_area_corner.map_or(selected_column_index + 1, |(x, _)| {
        if x > self.selected_column {
          (selected_column_index + 1).saturating_sub(x.saturating_sub(self.selected_column))
        } else {
          (selected_column_index + 1).saturating_add(self.selected_column.saturating_sub(x))
        }
      }),
...
impl StatefulDrawableComponent for TableComponent {
  fn draw<B: Backend>(&mut self, f: &mut Frame<B>, area: Rect, focused: bool) -> Result<()> {
    ...
    f.render_widget(
      Block::default().title(self.title()).borders(Borders::ALL).style(if focused {
        Style::default()
      } else {
    ...
    self.selected_row.selected().map_or_else(
      ...
      |selection| {
        self.scroll.update(selection, self.rows.len(), chunks[1].height.saturating_sub(2) as usize);
      },
    );
    ...
    TableValueComponent::new(self.selected_cells().unwrap_or_default()).draw(f, chunks[0], focused)?;

    TableStatusComponent::new(
      if self.rows.is_empty() { None } else { Some(self.rows.len()) },
      if self.headers.is_empty() { None } else { Some(self.headers.len()) },
...
impl Component for TableComponent {
  fn commands(&self, out: &mut Vec<CommandInfo>) {
    out.push(CommandInfo::new(command::extend_selection_by_one_cell(&self.key_config)));
  }
...
mod test {
  ...
  fn test_calculate_cell_widths_when_sum_of_cell_widths_is_greater_than_table_width() {
    ...
    component.headers = vec!["1", "2", "3"].iter().map(|h| h.to_string()).collect();
    component.rows = vec![
      vec!["aaaaa", "bbbbb", "ccccc"].iter().map(|h| h.to_string()).collect(),
      vec!["d", "e", "f"].iter().map(|h| h.to_string()).collect(),
    ];
    let (selected_column_index, headers, rows, constraints) = component.calculate_cell_widths(10);
    assert_eq!(selected_column_index, 1);
    assert_eq!(headers, vec!["", "1", "2"]);
    assert_eq!(rows, vec![vec!["1", "aaaaa", "bbbbb"], vec!["2", "d", "e"]]);
    assert_eq!(constraints, vec![Constraint::Length(1), Constraint::Length(5), Constraint::Min(10),]);
  }
  ...
  fn test_calculate_cell_widths_when_sum_of_cell_widths_is_less_than_table_width() {
    ...
    let (selected_column_index, headers, rows, constraints) = component.calculate_cell_widths(20);
    assert_eq!(selected_column_index, 1);
    assert_eq!(headers, vec!["", "1", "2", "3"]);
    assert_eq!(rows, vec![vec!["1", "aaaaa", "bbbbb", "ccccc"], vec!["2", "d", "e", "f"]]);
  ...
  fn test_calculate_cell_widths_when_component_has_multiple_rows() {
    ...
    component.rows = vec![
      vec!["aaaaa", "bbbbb", "ccccc"].iter().map(|h| h.to_string()).collect(),
      vec!["dddddddddd", "e", "f"].iter().map(|h| h.to_string()).collect(),
    ];

    let (selected_column_index, headers, rows, constraints) = component.calculate_cell_widths(20);
    assert_eq!(selected_column_index, 1);
    assert_eq!(headers, vec!["", "1", "2", "3"]);
    assert_eq!(rows, vec![vec!["1", "aaaaa", "bbbbb", "ccccc"], vec!["2", "dddddddddd", "e", "f"]]);
    assert_eq!(constraints, vec![
      Constraint::Length(1),

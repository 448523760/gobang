...
use super::{
  utils::scroll_vertical::VerticalScroll, Component, DatabaseFilterComponent, DrawableComponent, EventState,
};
use crate::{
...
impl DatabasesComponent {
  ...
  pub async fn update(&mut self, connection: &Connection, pool: &Box<dyn Pool>) -> Result<()> {
    let databases = match &connection.database {
      Some(database) => vec![Database::new(database.clone(), pool.get_tables(database.clone()).await?)],
      ...
      if item.kind().is_table() && name.contains(&filter) {
        ...
        return Spans::from(vec![
          Span::styled(
            ...
            if selected { Style::default().bg(Color::Blue) } else { Style::default() },
          ),
          Span::styled(
            middle.to_string(),
  ...
  fn draw_tree<B: Backend>(&self, f: &mut Frame<B>, area: Rect, focused: bool) -> Result<()> {
    f.render_widget(
      Block::default().title("Databases").borders(Borders::ALL).style(if focused {
        Style::default()
      } else {
    ...
    let tree = if let Some(tree) = self.filterd_tree.as_ref() { tree } else { &self.tree };
    ...
    let items = tree.iterate(self.scroll.get_top(), tree_height).map(|(item, selected)| {
      Self::tree_item_to_span(
        item.clone(),
        selected,
        area.width,
        if self.filter.input_str().is_empty() { None } else { Some(self.filter.input_str()) },
      )
    });
...
impl Component for DatabasesComponent {
  ...
  fn event(&mut self, key: Key) -> Result<EventState> {
    ...
    match key {
      ...
      key => {
        if tree_nav(
          if let Some(tree) = self.filterd_tree.as_mut() { tree } else { &mut self.tree },
          key,
          &self.key_config,
        ) {
          return Ok(EventState::Consumed);
...
mod test {
  ...
  fn test_tree_database_tree_item_to_span() {
    ...
    assert_eq!(
      DatabasesComponent::tree_item_to_span(
        DatabaseTreeItem::new_database(&Database { name: "foo".to_string(), children: Vec::new() }, false,),
        false,
        WIDTH,
        None,
      ),
      Spans::from(vec![Span::raw(format!("\u{25b8}{:w$}", "foo", w = WIDTH as usize))])
    );
  ...
  fn test_tree_table_tree_item_to_span() {
    ...
    assert_eq!(
      DatabasesComponent::tree_item_to_span(
        DatabaseTreeItem::new_table(&Database { name: "foo".to_string(), children: Vec::new() }, &Table {
          name: "bar".to_string(),
          create_time: None,
          update_time: None,
          engine: None,
      ...
      Spans::from(vec![Span::raw(format!("  {:w$}", "bar", w = WIDTH as usize))])
    );

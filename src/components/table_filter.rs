...
impl TableFilterComponent {
  ...
  fn update_completion(&mut self) {
    let input = &self
      ...
      .map(|i| i.to_string())
      .collect::<Vec<String>>();
    self.completion.update(input.last().unwrap_or(&String::new()));
  }
  ...
  fn complete(&mut self) -> anyhow::Result<EventState> {
    if let Some(candidate) = self.completion.selected_candidate() {
      ...
      let middle = if is_last_word {
        candidate.chars().map(|c| c.to_string()).collect::<Vec<String>>()
      } else {
        let mut c = candidate.chars().map(|c| c.to_string()).collect::<Vec<String>>();
        c.push(" ".to_string());
        c
      };
      ...
      self.input_cursor_position += middle.join("").chars().map(compute_character_width).sum::<u16>();
      if is_last_word {
        self.input_cursor_position += " ".to_string().width() as u16
      }
      self.input_cursor_position -= self.completion.word().chars().map(compute_character_width).sum::<u16>();
...
impl StatefulDrawableComponent for TableFilterComponent {
  fn draw<B: Backend>(&mut self, f: &mut Frame<B>, area: Rect, focused: bool) -> Result<()> {
    let query = Paragraph::new(Spans::from(vec![
      Span::styled(
        self.table.as_ref().map_or("-".to_string(), |table| table.name.to_string()),
      ...
      Key::Left => {
        if !self.input.is_empty() && self.input_idx > 0 {
          ...
          self.input_cursor_position =
            self.input_cursor_position.saturating_sub(compute_character_width(self.input[self.input_idx]));
          self.completion.update("");
        }
        Ok(EventState::Consumed)
      },
...
mod test {
  ...
  fn test_complete() {
    ...
    assert_eq!(filter.input, vec!['A', 'N', 'D', ' ', 'c', 'd', 'e', 'f', 'g']);
  }
  ...
  fn test_complete_end() {
    ...
    assert!(filter.complete().is_ok());
    assert_eq!(filter.input, vec!['a', 'b', ' ', 'c', 'd', 'e', 'f', ' ', 'I', 'N', ' ']);
  }

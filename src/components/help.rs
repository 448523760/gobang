...
impl HelpComponent {
  pub const fn new(key_config: KeyConfig) -> Self {
    Self { cmds: vec![], visible: false, selection: 0, key_config }
  }
  ...
  fn scroll_selection(&mut self, inc: bool) {
    ...
    new_selection = if inc { new_selection.saturating_add(1) } else { new_selection.saturating_sub(1) };
  ...
  fn get_text(&self, width: usize) -> Vec<Spans> {
    ...
    for (key, group) in &self.cmds.iter().group_by(|e| e.text.group) {
      txt.push(Spans::from(Span::styled(key.to_string(), Style::default().add_modifier(Modifier::REVERSED))));
      ...
      for command_info in group {
        ...
        txt.push(Spans::from(Span::styled(
          ...
          if is_selected { Style::default().bg(Color::Blue) } else { Style::default() },
        )));
      }
    }
...
mod test {
  ...
  fn test_get_text() {
    ...
    assert_eq!(component.get_text(width), vec![
      Spans::from(Span::styled("-- General --", Style::default().add_modifier(Modifier::REVERSED))),
      Spans::from(Span::styled(" Scroll up/down/left/right [k,j,h,l]  3", Style::default().bg(Color::Blue))),

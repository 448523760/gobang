...
impl PropertiesComponent {
  ...
  pub async fn update(&mut self, database: Database, table: Table, pool: &Box<dyn Pool>) -> Result<()> {
    self.column_table.reset();
    let columns = pool.get_columns(&database, &table).await?;
    if !columns.is_empty() {
      self.column_table.update(
        columns.iter().map(|c| c.columns()).collect::<Vec<Vec<String>>>(),
    ...
    if !constraints.is_empty() {
      self.constraint_table.update(
        constraints.iter().map(|c| c.columns()).collect::<Vec<Vec<String>>>(),
    ...
    if !foreign_keys.is_empty() {
      self.foreign_key_table.update(
        foreign_keys.iter().map(|c| c.columns()).collect::<Vec<Vec<String>>>(),
    ...
    if !indexes.is_empty() {
      self.index_table.update(
        indexes.iter().map(|c| c.columns()).collect::<Vec<Vec<String>>>(),
  ...
  fn tab_names(&self) -> Vec<(Focus, String)> {
    vec![
      ...
      (Focus::Constraint, command::tab_constraints(&self.key_config).name),
      (Focus::ForeignKey, command::tab_foreign_keys(&self.key_config).name),
...
impl Component for PropertiesComponent {
  fn commands(&self, out: &mut Vec<CommandInfo>) {
    out.push(CommandInfo::new(command::toggle_property_tabs(&self.key_config)));
  }

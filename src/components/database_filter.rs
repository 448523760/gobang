...
impl DatabaseFilterComponent {
  pub fn new() -> Self {
    Self { table: None, input: Vec::new(), input_idx: 0, input_cursor_position: 0 }
  }
...
impl DrawableComponent for DatabaseFilterComponent {
  fn draw<B: Backend>(&self, f: &mut Frame<B>, area: Rect, focused: bool) -> Result<()> {
    let query = Paragraph::new(Spans::from(format!(
      "{:w$}",
      if self.input.is_empty() && !focused { "Filter tables".to_string() } else { self.input_str() },
    ...
    if focused {
      f.set_cursor((area.x + self.input_cursor_position).min(area.right().saturating_sub(1)), area.y)
    }
    Ok(())
  }
}
